...
const Sidebar = () => {
    const user=useSelector(state=>state?.user)   // getting user detail for avatar name
    const [editUserOpen,setEditUserOpen]=useState(false)
    const [allUser,setAllUser]=useState([])
    const [operSearchUser, setOpenSearchUser] =useState(false)
             ...
             <div className='h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar'>
             {
              allUser.length===0 &&(
                <div className='mt-12'>
                  ...
                  <p className='text-lg text-center text-slate-400'>Explore user to start the conversation with.</p>
                </div>
              )
             }
             {
